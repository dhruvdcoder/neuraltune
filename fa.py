...
def search(X, step_size, chunk_size, pool_workers):
    score = partial(compute_score, X)
    grid = list(range(1, 50, step_size))
...
def fa(x, step_size=1, chunk_size=2, pool_workers=5):
    x = np.asarray(x)
    n_components_fa = search(x, step_size=step_size, chunk_size=chunk_size, pool_workers=pool_workers)
...
if __name__=='__main__':
    ...
    X_transformed = fa(X, step_size=args.step, chunk_size=args.chunk, pool_workers=args.workers)
